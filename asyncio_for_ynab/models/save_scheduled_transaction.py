...
class SaveScheduledTransaction(BaseModel):
    ...
    """ # noqa: E501
    account_id: UUID
    var_date: date = Field(description="The scheduled transaction date in ISO format (e.g. 2016-12-01).  This should be a future date no more than 5 years into the future.", alias="date")
    amount: Optional[StrictInt] = Field(default=None, description="The scheduled transaction amount in milliunits format.")
    payee_id: Optional[UUID] = Field(default=None, description="The payee for the scheduled transaction.  To create a transfer between two accounts, use the account transfer payee pointing to the target account.  Account transfer payees are specified as `transfer_payee_id` on the account resource.")
    ...
    category_id: Optional[UUID] = Field(default=None, description="The category for the scheduled transaction. Credit Card Payment categories are not permitted. Creating a split scheduled transaction is not currently supported.")
    memo: Optional[Annotated[str, Field(strict=True, max_length=500)]] = None
