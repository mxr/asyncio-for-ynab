# asyncio_for_ynab.CategoriesApi
...
async with asyncio_for_ynab.ApiClient(configuration) as api_client:
    ...
    api_instance = asyncio_for_ynab.CategoriesApi(api_client)
    ...
    data = asyncio_for_ynab.PostCategoryWrapper() # PostCategoryWrapper | The category to create.

    try:
        # Create a category
        api_response = await api_instance.create_category(plan_id, data)
...
async with asyncio_for_ynab.ApiClient(configuration) as api_client:
    ...
    api_instance = asyncio_for_ynab.CategoriesApi(api_client)
    plan_id = 'plan_id_example' # str | The id of the plan (\"last-used\" can be used to specify the last used plan and \"default\" can be used if default plan selection is enabled (see: https://api.ynab.com/#oauth-default-plan)
    data = asyncio_for_ynab.PostCategoryGroupWrapper() # PostCategoryGroupWrapper | The category group to create.

    try:
        # Create a category group
        api_response = await api_instance.create_category_group(plan_id, data)
...
async with asyncio_for_ynab.ApiClient(configuration) as api_client:
    ...
    try:
        # Get all categories
        api_response = await api_instance.get_categories(plan_id, last_knowledge_of_server=last_knowledge_of_server)
...
async with asyncio_for_ynab.ApiClient(configuration) as api_client:
    ...
    try:
        # Get a category
        api_response = await api_instance.get_category_by_id(plan_id, category_id)
...
async with asyncio_for_ynab.ApiClient(configuration) as api_client:
    ...
    try:
        # Get a category for a specific plan month
        api_response = await api_instance.get_month_category_by_id(plan_id, month, category_id)
...
async with asyncio_for_ynab.ApiClient(configuration) as api_client:
    ...
    plan_id = 'plan_id_example' # str | The id of the plan. \"last-used\" can be used to specify the last used plan and \"default\" can be used if default plan selection is enabled (see: https://api.ynab.com/#oauth-default-plan).
    category_id = 'category_id_example' # str | The id of the category
    data = asyncio_for_ynab.PatchCategoryWrapper() # PatchCategoryWrapper | The category to update

    try:
        # Update a category
        api_response = await api_instance.update_category(plan_id, category_id, data)
...
async with asyncio_for_ynab.ApiClient(configuration) as api_client:
    ...
    data = asyncio_for_ynab.PatchCategoryGroupWrapper() # PatchCategoryGroupWrapper | The category group to update

    try:
        # Update a category group
        api_response = await api_instance.update_category_group(plan_id, category_group_id, data)
...
async with asyncio_for_ynab.ApiClient(configuration) as api_client:
    ...
    month = '2013-10-20' # date | The plan month in ISO format (e.g. 2016-12-01) (\"current\" can also be used to specify the current calendar month (UTC))
    category_id = 'category_id_example' # str | The id of the category
    data = asyncio_for_ynab.PatchMonthCategoryWrapper() # PatchMonthCategoryWrapper | The category to update.  Only `budgeted` (assigned) amount can be updated and any other fields specified will be ignored.

    try:
        # Update a category for a specific month
        api_response = await api_instance.update_month_category(plan_id, month, category_id, data)
